...
description: "Orchestrate delegated maintainer work across Gil's GitHub repositories: prepare decision-ready PRs, monitor workers, clear queues, and release."
---
...
- Use repositories owned by Gil's authenticated GitHub account.
- Resolve the account with `gh api user --jq .login`.
- Resolve active repositories with `gh repo list "$(gh api user --jq .login)" --limit 200 --json nameWithOwner,isArchived,url`.
- Exclude archived repositories unless Gil explicitly names one.
- Include repositories outside that account only when Gil explicitly names them.
