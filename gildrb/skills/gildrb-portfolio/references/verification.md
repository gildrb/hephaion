...
- In `scripts/verify-page.mjs`, assert exactly one `View next` block and every other configured project row on every case route; reject self-route hrefs, unknown routes, stale target metadata, and incorrect homepage order. Assert that `/all` contains zero `case-next` markup.
