...
- Put exactly one `<!-- @case-next -->` token after the article and inside the same content column. The builder replaces it with the generated `Read next` navigation; do not put suggestion markup in Markdown or templates.
