...
- Put exactly one `<!-- @case-next -->` token after the article and inside the same content column. The builder replaces it with every other project as generated `Read next` anchors; the first three are visible without JavaScript, and client selection may reveal a different three without creating rows. Do not put suggestion markup in Markdown or templates.
