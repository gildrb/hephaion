...
- Put exactly one `<!-- @case-next -->` token after the article and inside the same content column. The builder replaces it with the complete homepage project table; the current project is plain `aria-current="page"` text and other rows are generated anchors. Do not put suggestion markup in Markdown or templates.
