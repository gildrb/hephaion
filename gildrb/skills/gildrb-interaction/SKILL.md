...
- The shared sticky header uses an always-opaque page-background band and a scroll-conditional gradient below its bottom edge, transitioning in only after content scrolls beneath it.
...
- The mobile homepage table remains in normal document flow below the shared sticky header. Do not add a locked viewport, nested table scroll, separately sticky filter row, scroll fades, scrollbar suppression, or scroll-chaining workaround.
...
- An entry animation that can strand the page hidden, replays on sort or theme change, or runs on a case route.
