...
- Keep captions concise and evidence-specific only when the user supplied or explicitly authorized them; do not enforce a hard word limit on approved copy.
- A tall single-image specimen may be sliced into sequential full-width panels only on empty background. Slicing is not cropping; no panel may lose edge content.
