...
- The mobile layout container has no vertical padding. The name supplies `24px` top and `8px` bottom padding. On the homepage, every case-study route, and `/all`, the shared header keeps the same sticky `56px` row height in both themes and centers both icon sizes inside its content area so switching cannot move or resize the button. It shifts that shared icon center `2px` lower with `--theme-toggle-optical-offset`, using `26px` top and `6px` bottom padding for optical alignment with the first `Gil Rodrigues` line. `.content` uses `display: contents` with `0` padding. The name and theme control stay sticky while the document scrolls normally beneath them; keep native root overscroll available for pull-to-refresh. Keep the opaque page-background band always present, but show the gradient below it only while `has-sticky-header-fade` reflects a nonzero document scroll position, with a transition. Recompute that state after scroll, resize, and back/forward restoration.
...
- On desktop, apply the existing footer/theme-toggle endpoint calculation to the final table row's bottom padding. At maximum scroll, the last row's text center aligns with the theme-toggle center while preserving the content column's 48px bottom padding. This is a maximum endpoint only; short pages remain natural flow.
- On the mobile homepage, keep the project table in normal document flow beneath the shared sticky header. Do not lock the viewport, create an inner scroll region, pin the filter row separately, add scroll fades, suppress the scrollbar, or add scroll-chaining workarounds. Leave document overflow and `overscroll-behavior` native so page scrolling and pull-to-refresh remain available; the Links and Contact block follows the final project row as the end of the page.
