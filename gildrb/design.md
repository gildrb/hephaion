...
- On the locked mobile homepage, keep the table header sticky while its rows scroll. Give the header an opaque page-background backdrop, then reuse the case-page `56px` gradient overlay treatment below the header and above the footer edge for top and bottom scroll fades, enabling each fade only when that direction has overflow. Extend the bottom fade through the table's `8px` trailing padding so it reaches the real clipping edge. Suppress the native overlay scrollbar so it cannot cover the arrow column.
