...
- On the locked mobile homepage, keep the filter row in the non-scrolling `.portfolio-scroll-frame` wrapper, outside the scrolling `.portfolio-section`; the wrapper reserves the header row and the table rows own the only inner scroll region. Give the header an opaque page-background backdrop, keep the table's `overscroll-behavior` at `auto` so nested scrolling does not suppress native pull-to-refresh, and leave document overflow visible so the browser retains its elastic pull-to-refresh gesture while the `100dvh` grid keeps document height fitted. Then reuse the case-page `56px` gradient overlay treatment below the header and above the footer edge for top and bottom scroll fades, enabling each fade only when that direction has overflow. Suppress the native overlay scrollbar so it cannot cover the arrow column.
