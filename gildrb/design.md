...
- On the mobile homepage, keep the name and theme toggle in document flow while Links/Contact follows the table. On case-study routes and `/all`, retain the existing sticky location row and theme control. Use native document scrolling so keyboard, screen-reader, and touch access traverse the complete page.
