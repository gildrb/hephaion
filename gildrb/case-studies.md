...
The block begins 48px after the article, matching the existing `###` heading rhythm. Its rows use the homepage table's four-column treatment and separators, without sort controls or the `All` column. On desktop, the final row receives the existing footer/theme-toggle endpoint calculation as bottom padding, so its text center aligns with the toggle at maximum scroll while the content column keeps its 48px bottom padding. This is a maximum endpoint only; short pages are not stretched. Mobile keeps the 48px flow gap and allows the table to scroll internally only when the locked viewport cannot fit it.
