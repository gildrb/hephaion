...
Every generated case-study route ends with a build-generated `View next` table containing every other configured project exactly once, in the homepage's default newest-first order. No row links to the current project. Date, title, and scope metadata come from the parsed homepage rows; do not hand-write a second project registry. The `/all` continuous reading route deliberately has no `View next` block.

The block begins 48px after the article, matching the existing `###` heading rhythm. Its rows use the homepage table's four-column treatment and separators, without sort controls or the `All` column. On desktop, the table container receives the footer/theme-toggle endpoint calculation as bottom padding while the final row stays unpadded, so the row bottom aligns with the toggle at maximum scroll while the content column keeps its 48px bottom padding. This is a maximum endpoint only; short pages are not stretched. On mobile, keep the 48px article-to-table flow gap and `var(--section-gap)` between the table container and the shared Links/Contact block.
