...
The block begins 48px after the article, matching the existing `###` heading rhythm. Its rows use the homepage table's four-column treatment and separators, without sort controls or the `All` column. The heading remains in normal document flow, and the content column keeps its 48px bottom padding. Mobile keeps the 48px flow gap and allows the table to scroll internally only when the locked viewport cannot fit it.
